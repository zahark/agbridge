...
| a bare `agb-refresh` exits non-zero saying `no bridge was started again for: <label>` | that instance's rows were forgotten and `launchctl` then refused both `bootstrap` and `load -w`, so its sidebar has nothing to re-mint it | what the tool prints, and it branches on the label: for `com.agbridge.<name>`, re-run `install.sh mac --instance <name>`; for a **custom** label it says `--instance <name>` as a placeholder, because that install's name is its label and typing it would make a *new* instance; for bare `com.agbridge` it says the opposite — that is a legacy nameless install, a re-run **mints a second instance beside it**, so load the plist by hand or migrate it to a name (`CHANGELOG.md`, *Upgrading from ≤ 0.5.0*). This is exit **4** and is always an error, `--key` or not |
