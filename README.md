...
| `agb-claude [name]` | cluster | helper: starts Claude Code in a **named tmux session**, so its row is attachable — and mints the row **before Claude starts**, so it exists even while Claude sits on a trust prompt. `-d` starts it in the background. `AGB_CLAUDE_CUSTOM` replaces the `claude` command line — `{}` splices this run's flags, `{env}` carries the identity a Claude on **another machine** must report or it mints a second row |
...
| `agb-codex [name]` | cluster | helper: the same for [Codex](https://github.com/openai/codex). ⚠️ Codex fires no agbridge hooks, so the row it mints never leaves `completed`. `AGB_CODEX_CUSTOM` replaces the `codex` command line, for starting the agent through a scheduler, container or pool launcher; `{}` splices this run's flags into it |
...
| `notify_on_new_row` | `bridge` | desktop banner when an agent appears that had no row — naming its label, host and directory. **On by default**; `0`, `no`, `off` or `false` disables it. Rows minted in a burst (a bridge start, a reconnect, `agb-refresh`) are silent for 3 seconds, or a nine-row refresh would be nine banners. It also takes a **comma-separated list of states** (`active`, `blocked`, `completed`), and then only an agent whose **first-seen** state is one of them is announced. `notify_on_new_row = completed` is the useful one: `agb-claude` mints the row before Claude starts, with `completed`, while a bare `claude` cannot arrive as anything but `active` — so it reads as *announce the sessions I started on purpose, not every `claude` somebody runs on the cluster*. `idle` is refused (no new row can arrive in it), and any unknown state refuses the whole list and falls back to **on**, with the reason in the bridge log |
...
| `row_fields` | `bridge` | which fields a row's title shows, and in what order: any of `label`, `host`, `cwd`, `pane`, `beat`, `key`, comma-separated, with `cwd:base` for the directory's basename alone. An unknown name refuses the **whole** list and logs why. Default `label,host,cwd,pane,beat` — exactly today's title |
...
```sh
python3 -m pytest tests/ -q          # no network, no second host, no Mac required
...
**Running end to end against a live agterm**, across two Linux hosts and a Mac. The test suite
needs no network and no second machine to run.
