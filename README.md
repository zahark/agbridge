...
| `notify_on_new_row` | `bridge` | desktop banner when an agent appears that had no row — naming its label, host and directory. **On by default**. Rows minted in a burst (a bridge start, a reconnect, `agb-refresh`) are silent for `NEW_ROW_QUIET` seconds, or a nine-row refresh would be nine banners |
