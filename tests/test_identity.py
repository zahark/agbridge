...
def test_tmux_anchor_parses_the_server_pid_and_the_pane(agb, set_tmux):
    ...
    assert agb.tmux_anchor_parts() == (1200000, "%24")
...
@pytest.mark.parametrize("tmux", [
    "",
    "/tmp/tmux-100000/default",
    "/tmp/tmux-100000/default,1200000",       # session field missing
    "/tmp/tmux-100000/default,notapid,23",
    "/tmp/tmux-100000/default,0,23",
    "/tmp/tmux-100000/default,-5,23",
])
...
def test_tmux_anchor_is_host_serverpid_pane(agb, set_tmux):
    ...
    assert (anchor.kind, anchor.spid, anchor.pane) == ("tmux", 1200000, "%24")
    assert anchor.name() == "box2-1200000-%24"
...
def test_idx_name_round_trips_through_a_dashed_hostname(agb):
    anchor = agb.Anchor("worker01", "tmux", 1200000, "%24",
                        pane="%24")
    name = anchor.name()
    assert name == "worker01-1200000-%24"
...
def test_two_panes_in_one_tmux_session_get_two_distinct_keys(agb, statedir,
    ...
    assert sorted(os.listdir(str(statedir / "idx"))) == [
        "box2-1200000-%24", "box2-1200000-%25"]
...
def _mint_worker(agb_path, sd, host, pid, starttime, barrier, queue):
    ...
    anchor = module.Anchor(host, "tmux", 1200000, "%24", pane="%24")
